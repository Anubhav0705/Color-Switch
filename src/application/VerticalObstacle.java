...
public class VerticalObstacle extends Obstacle{
    ...
    //ColorSwitch ch;
    public VerticalObstacle(double x,double y,int id,double size,double duration) {
	...
	public void update() {
		...
		KeyValue kv=new KeyValue(group.layoutXProperty(),group.getLayoutX()+300);
		...
		KeyValue kv2=new KeyValue(group2.layoutXProperty(),group2.getLayoutX()-300);
	...
	@Override
	public void draw() {
		rect1=new Rectangle(positionX,positionY,20,size);
		rect2=new Rectangle(positionX+80,positionY,20,size);
		rect3=new Rectangle(positionX+160,positionY,20,size);
		rect4=new Rectangle(positionX+240,positionY,20,size);
		rect5=new Rectangle(positionX+350,positionY,20,size);
		rect6=new Rectangle(positionX+430,positionY,20,size);
		rect7=new Rectangle(positionX+510,positionY,20,size);
		rect8=new Rectangle(positionX+580,positionY,20,size);
	...
	public void isCollisionObstacle(Ball ball) {
		...
		if(collision1.getBoundsInLocal().getWidth()!=-1 || collision8.getBoundsInLocal().getWidth()!=-1) {
			if(!(ball.getColor().equals(Color.YELLOW)  ) ){

				ball.setCollision(true);
				group.setLayoutY(10000);
				group2.setLayoutY(10000);
			}
			
		}
		if(collision2.getBoundsInLocal().getWidth()!=-1 || collision7.getBoundsInLocal().getWidth()!=-1) {
...
//				System.out.println("ANUBJDHJHJJHDJHKHHSGKD");
		...
		if(collision3.getBoundsInLocal().getWidth()!=-1 || collision6.getBoundsInLocal().getWidth()!=-1) {
...
//				System.out.println("ANUBJDHJHJJHDJHKHHSGKD");
		...
		if(collision4.getBoundsInLocal().getWidth()!=-1 || collision5.getBoundsInLocal().getWidth()!=-1) {
